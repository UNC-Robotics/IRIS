...
#if USE_HEURISTIC
        new_node->SetHeuristic(map_->ComputeHeuristic(v, new_node->VisSet(), graph_, virtual_graph_coverage_));
#endif
...
void GraphSearch::GreedySearchForFullCoverage() {
    const TimePoint start = Clock::now();

    Idx source = 0;
    auto max_coverage = virtual_graph_coverage_.Size();
    ...
    while(!valid_result_found) {
        ...
        while(true) {
            // check for termination
            if (current_node->VisSet().Size() == max_coverage) {
                found = true;
                break;
            }

            // extend a node
            auto first_successor = ComputeNearestSuccessor(current_node);
            current_node = first_successor;
        }
    ...
    // std::cout << "Greedy: " << current_node->CostToCome() << ", "
    // << current_node->VisSet().Size()/(RealNum)MAX_COVERAGE_SIZE << std::endl;

    greedy_cost_ = current_node->CostToCome();
}
...
bool GraphSearch::DominatedByOpenStateCompleteLazy(const NodePtr node, const bool skip_queue_operations) {
    // this is a copy
    auto index = node->Index();
...
void GraphSearch::RecycleSubsumedNodes(NodePtr node, std::queue<NodePtr>& recycle_bin, const bool force_recycle_all) {
